...
########################################### QUELLE ###################################################
    ...
    def segmentation(self, img):
        ...
        # black and white image (2D Array): 255 (>50) => NOT in color range, 0 to 50 => IN color range
        # line in received image is black = (0, 0, 0)
        # background in received image is grey = (74, 74, 74)
        # >50 will be set to 0 (black) => background will be set to black.. sorry
        # 0 to 50 will be set to 1 -> 255 (white) => line will be set to white.. sorry again
    ...
    # first started counting the first ten rows -- changed to just one row, hence the for-loop dummy
    ...
    def get_line_state(self, img):
        ...
        # flip image vertically (pixel on the right will be on the left, pixel on top stays on top)
        ...
        if (left >= (width * (99.0 / 100.0)) or right >= (width * (99.0 / 100.0))):
            ...
            state = 7
        elif (middle >= (width * (0.0 / 100.0)) and middle <= (width * (2.5 / 100.0))):
            # line is far left
            state = 0
        elif (middle > (width * (2.5 / 100.0)) and middle <= (width * (21.5 / 100.0))):
            # line is left
            state = 1
        elif (middle > (width * (21.5 / 100.0)) and middle <= (width * (40.5 / 100.0))):
            # line is slightly left
            state = 2
        elif (middle > (width * (40.5 / 100.0)) and middle <= (width * (59.5 / 100.0))):
            # line is in the middle
            state = 3
        elif (middle > (width * (59.5 / 100.0)) and middle <= (width * (78.5 / 100.0))):
            # line is slightly right
            state = 4
        elif (middle > (width * (78.5 / 100.0)) and middle <= (width * (97.5 / 100.0))):
            # line is right
            state = 5
        elif (middle * (97.5 / 100.0)) and middle <= (width * (100.0 / 100.0)):
            # line is far right
            state = 6
        else:
            # line is lost
